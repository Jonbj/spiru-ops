# TODO — Azioni pendenti

## Urgente
- [ ] Attendere risposta email ACUF (info@acuf.net) → [[fornitori/ACUF]]
- [ ] Attendere risposta email BEA → [[fornitori/BEA]]
- [ ] Misurare diametro interno collo damigiana 50L (verifica stick LED)

## Hardware
- [ ] Ordinare DFRobot SEN0169-V2 (pH industriale) ~35–45€
- [ ] Ordinare DFRobot DFR0300 IP68 (EC K=1) ~50–60€
- [ ] Ordinare DS18B20 impermeabile ~3–5€
- [ ] Ordinare D4184 MOSFET module per dimmer LED ESP32
- [ ] Tappo silicone con foro (alcofermbrew.com) misura collo damigiana

## Coltura
- [ ] Rimuovere copertura vimini dalle damigiane grandi
- [ ] Preparare mezzo Zarrouk semplificato per 50L
- [ ] Definire protocollo scale-up da vial a 50L damigiana

## Pipeline spiru-ops
- [ ] Lanciare run con nuovi focus (equipment_sanitization_cip_spirulina)
- [ ] Verificare seed manuali inoculo e novel food nei prossimi run
- [ ] Valutare implementazione query expansion LLM nel copilot

## Amministrativo
- [ ] Scrivere email ACUF — FATTO ✅
- [ ] Scrivere email BEA — FATTO ✅
